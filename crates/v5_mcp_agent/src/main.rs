//! v5_mcp_agent - MCP Browser Support - Mini Claude Code: Skills Mechanism (~1500 lines)
//!
...
//!
//! Usage:
//!     cargo run -p v5_mcp_agent - MCP Browser Support
...
mod mcp_client;
use mcp_client::McpBrowserClient;
...
fn create_base_tools() -> Vec<Tool> {
    vec![
        ...
        Tool {
            ...
            description: "Navigate to a URL in the browser. Requires Chrome/Edge with remote debugging enabled on port 9222.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "url": {
        ...
        Tool {
            ...
            description: "Take a screenshot of the current browser page. Returns the screenshot as base64 image data.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {},
                "required": []
        ...
        Tool {
            name: "browser_get_performance".to_string(),
            description: "Get performance metrics for the current page (load time, resource counts, etc.).".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {},
                "required": []
        ...
        Tool {
            ...
            description: "Take a text snapshot of the current page content based on accessibility tree. This shows all interactive elements and text content with unique IDs.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {},
                "required": []
...
// =============================================================================
// Browser Automation Functions (MCP)
// =============================================================================

fn run_browser_navigate(mcp_client: &Arc<Mutex<McpBrowserClient>>, url: &str) -> String {
    let client = mcp_client.lock().unwrap();
    match client.navigate(url) {
        Ok(result) => result,
        Err(e) => format!("Error navigating to {}: {}", url, e),
    }
}

fn run_browser_screenshot(mcp_client: &Arc<Mutex<McpBrowserClient>>) -> String {
    let client = mcp_client.lock().unwrap();
...
fn run_browser_get_performance(mcp_client: &Arc<Mutex<McpBrowserClient>>) -> String {
    let client = mcp_client.lock().unwrap();
    match client.get_performance() {
        Ok(result) => result,
        Err(e) => format!("Error getting performance metrics: {}", e),
    }
}

fn run_browser_snapshot(mcp_client: &Arc<Mutex<McpBrowserClient>>) -> String {
    let client = mcp_client.lock().unwrap();
    match client.take_snapshot() {
        Ok(result) => result,
...
async fn run_task(
    ...
    mcp_client: &Arc<Mutex<McpBrowserClient>>,
    description: &str,
    prompt: &str,
    agent_type: &str,
) -> String {
    ...
    match name {
        ...
        "browser_navigate" => {
            if let Some(url) = input.get("url").and_then(|v| v.as_str()) {
                run_browser_navigate(mcp_client, url)
            } else {
...
async fn execute_tool_async(
    ...
    if name == "Task" {
        ...
        execute_tool(config, todo_manager, skill_loader, mcp_client, name, input)
    }
}
...
async fn main() -> Result<()> {
    ...
    let mcp_available = mcp_client.lock().unwrap().check_available();
    if mcp_available {
        println!("{} chrome-devtools-mcp detected", "✓".bright_green());
        // Start MCP server
        if let Err(e) = mcp_client.lock().unwrap().start() {
            println!("{} Failed to start MCP server: {}", "⚠".bright_yellow(), e);
            println!("{} Browser tools will be unavailable", "⚠".bright_yellow());
        }
    } else {
        println!("{} chrome-devtools-mcp not found", "⚠".bright_yellow());
        println!(
    ...
    println!(
        ...
        "v5".bright_magenta(),
        "(Skills + Subagents + Todo + Browser)".bright_black()
    );
    println!("{}", "=".repeat(60).bright_black());
    ...
    if mcp_available {
        println!("{} Browser automation enabled", "Browser:".bright_green());
    } else {
        println!("{} Browser automation disabled", "Browser:".bright_yellow());
    }
    ...
    loop {
        ...
        if let Err(e) =
            agent_loop(&client, &config, &skill_loader, &mcp_client, &mut messages).await
        {
